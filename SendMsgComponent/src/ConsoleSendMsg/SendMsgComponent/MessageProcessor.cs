    ...
    class MessageProcessor : IMessageProcessor
    {
        ...
        public void Register(MessageContext context)
        {
            try
            {
                ...
                Logger.WriteInfoFmt(Log.SenderRegister, "Key:{0},地址个数:{1},队列大小:{2}", new object[] { context.Key, context.Address.Length, context.QueueSize });
            }
            catch (Exception ex)
            {
        ...
        public void AddMessage(string key, IMessage imessage)
        {
            ...
            {
                //获取消息
                byte[] msgByte = MessageFactory.Serialize(imessage);
                //插入消息
